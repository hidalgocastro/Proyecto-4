...
# ---
# 
# * Estudiante: **José Martí Hidalgo Castro**
# * Carné: **B63466**
# * Grupo: **1**
# 
# ---
...
# Solución 4.1 Modulación 16-QAM:
...
def modulador(bits, fc, mpp):
    ...
    bits2 = bits.reshape(-1, 4)

    # 2. Construcción del periodo de la señal portadora s(t)
    ...
    '''
     La modulación 16-QAM utiliza dos señales portadoras, 
    en este caso I y Q. Determinadas como:
    Portadora_I = sen(2*Pi*fc*T)
    Portadora_Q = cos(2*Pi*fc*T)
    '''
    
    Portadora_I = np.sin(2*np.pi*fc*t_periodo)
    Portadora_Q = np.cos(2*np.pi*fc*t_periodo)

    # 3. Estapa de inicio la señal modulada s(t)
    
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.zeros(t_simulacion.shape)
    moduladora = np.zeros(t_simulacion.shape)  
    # opcional:: señal de bits
 
    # 4. Asignar las formas de onda según los bits (16-QAM) (constelación). 
    
    i=0
    for b1, b2, b3, b4 in bits2:
        if b1 == 0 and b2 == 0:
            senal_Tx[i*mpp : (i+2)*mpp] = Portadora_Q * -3
            moduladora[i*mpp : (i+2)*mpp] = -3
        if b1 == 0 and b2 == 1:
            senal_Tx[i*mpp : (i+2)*mpp] = Portadora_Q * -1
        ...
        if b3 == 0 and b4 == 0:
            senal_Tx[(i+2)*mpp : (i+4)*mpp] = Portadora_I * 3
            moduladora[(i+2)*mpp : (i+4)*mpp] = 3
        if b3 == 0 and b4 == 1:
            senal_Tx[(i+2)*mpp : (i+4)*mpp] = Portadora_I* 1
...
def demodulador(senal_Rx, Portadora_I, Portadora_Q, mpp):
   ...
   # matriz = np.zeros((int(N/4),4))
    ...
    for i in range(0, N, 4):
        ...
        Producto_Q = (senal_Rx[i*mpp:(i+2)*mpp] * Portadora_Q)
        Ep_Q = np.sum(Producto_Q)

        # Reconstruyendo la parte coseno de la señal demodulada
        senal_demodulada[i*mpp:(i+2)*mpp] = Producto_Q
        
        # ---------------------------------------------------------
        
        # Producto interno de dos funciones para la portadora seno
        Producto_I = (senal_Rx[(i+2)*mpp:(i+4)*mpp] * Portadora_I)
        Ep_I = np.sum(Producto_I)

        # Reconstruyendo la parte seno de la señal demodulada
        senal_demodulada[(i+2)*mpp:(i+4)*mpp] = Producto_I
        
        # Criterio de decisión por detección de energía en portadora coseno Q
        if Ep_Q > 40:
            bits_Rx[i] = 1
            bits_Rx[i+1] = 0

        if Ep_Q > 0 and Ep_Q < 40:
            bits_Rx[i] = 1
            bits_Rx[i+1] = 1

        if Ep_Q > -40 and Ep_Q < 0:
            bits_Rx[i] = 0
            bits_Rx[i+1] = 1
        ...
        # Criterio de decisión por detección de energía en portadora seno I
        if Ep_I > 40:
            bits_Rx[i+2] = 0
            bits_Rx[i+3] = 0

        if Ep_I > 0 and Ep_I < 40:
            bits_Rx[i+2] = 0
            bits_Rx[i+3] = 1

        if Ep_I > -40 and Ep_I < 0:
            bits_Rx[i+2] = 1
            bits_Rx[i+3] = 1
...
#---------------------Método de recuperación de bits -----------------------------------------------------
...
#----------------------Método de impresión de la imagen Tx y la Rx------------------------------
...
# muestras por periodo de la portadora
mpp = 20
...
# 1. Importar y convertir la imagen a Tx
imagen_Tx = fuente_info('arenal.jpg')
dimensiones = imagen_Tx.shape
...
# 3. Modulación de la cadena de bits, en 16-QAM
senal_Tx, Pm, Portadora_I, Portadora_Q, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Tx de la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Desmodulación la señal con la imagen recibida del canal de Tx
bits_Rx, senal_demodulada = demodulador(senal_Rx, Portadora_I, Portadora_Q, mpp)

# 6. Visualización la imagen Rx
imagen_Rx = bits_a_rgb(bits_Rx, dimensiones)
Fig = plt.figure(figsize=(10,6))
...
# Visualizar el cambio entre las señales, por gráficas
fig, (ax1, ax2, ax3, ax4) = plt.subplots(nrows=4, sharex=True, figsize=(14, 7))
...
# La señal modulada al dejar el canal de Tx
ax3.plot(senal_Rx[0:600], color='b', lw=2) 
ax3.set_ylabel('$s(t) + n(t)$')
...
#4.2 Solución Estacionaridad y ergodicidad.

'''
Se importa: 
- librería NumPy.
- biblioteca "matplotlib", con lafunción "pyplot".
- módulo stats de la librería SciPy.
'''
...
# Funcion del tiempo S(t) con N realizaciones
...
'''
Se importa: 
- módulo  de lafft (función rápida de Fourier) librería SciPy.
'''
